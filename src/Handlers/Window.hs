module Handlers.Window (mkWindowHandler) where

import Control.Concurrent.MVar
import Control.Monad (msum, unless, when)
import Control.Monad.State hiding (state)
import Data.Bimap qualified as B
import Data.Map.Strict qualified as M
import Data.Maybe (fromMaybe)
import Data.Sequence qualified as S
import Data.Text qualified as T
import Optics.Core
import Optics.State
import Optics.State.Operators
import Protocols.Generated
import Types
import Utils.BiSeqMap qualified as BS
import Utils.Helpers
import Wayland.Connection

mkWindowHandler :: MVar WMState -> RiverWindowV1Handlers
mkWindowHandler mvar =
  RiverWindowV1Handlers
    { onRiverWindowV1CaptureSessions = \_ _ -> pure ()
    , onRiverWindowV1Closed = closed mvar
    , onRiverWindowV1AppId = newAppId mvar
    , onRiverWindowV1Dimensions = dimensions mvar
    , onRiverWindowV1DimensionsHint = dimHint mvar
    , onRiverWindowV1DecorationHint = \_ _ -> pure ()
    , onRiverWindowV1Title = newTitle mvar
    , onRiverWindowV1FullscreenRequested = fullReq mvar
    , onRiverWindowV1ExitFullscreenRequested = exitFullReq mvar
    , onRiverWindowV1Parent = newParent mvar
    , onRiverWindowV1PointerMoveRequested = \_ _ -> pure ()
    , onRiverWindowV1PointerResizeRequested = \_ _ _ -> pure ()
    , onRiverWindowV1Identifier = newIdent mvar
    , onRiverWindowV1MaximizeRequested = maximizeReq mvar
    , onRiverWindowV1UnmaximizeRequested = unmaximizeReq mvar
    , onRiverWindowV1MinimizeRequested = \_ -> pure ()
    , onRiverWindowV1ShowWindowMenuRequested = \_ _ _ -> pure ()
    , onRiverWindowV1PresentationHint = \_ _ -> pure ()
    , onRiverWindowV1UnreliablePid = \_ _ -> pure ()
    }

newIdent :: MVar WMState -> Object RiverWindowV1 -> T.Text -> W ()
newIdent mvar win ident = do
  modifyMVarW_ mvar $ pure . execState transform
 where
  transform = do
    #allWindows % at win %? #winIdentifier .= ident
    use (#persistedStateWindows % at ident) >>= \case
      Nothing -> #newWindowQueue %= (win :)
      Just (ws, status) -> do
        #persistedStateWindows % at ident .= Nothing
        fWs <- use (focusedWorkspace % non 1)
        unless (ws == fWs) $ #renderQueue >>>= riverWindowV1Hide win
        case status of
          Tiled -> do
            #allWorkspacesTiled %= BS.insert ws win
          Floating -> do
            #floatingQueue % at ws %?= (win :)
            #allWindows % at win %? #winFloat .= True
          Fullscreen -> do
            #fullscreenQueue % at ws %?= (win :)
            #allWindows % at win %? #winFull .= True
          FullscreenFloating -> do
            #fullscreenQueue % at ws %?= (win :)
            #allWindows % at win %? #winFloat .= True
            #allWindows % at win %? #winFull .= True

closed :: MVar WMState -> Object RiverWindowV1 -> W ()
closed mvar win = do
  modifyMVarW_ mvar $ \s -> do
    riverWindowV1Destroy win
    pure $ execState transform s
 where
  transform = do
    #allWindows %= M.delete win
    #workspaceFocusHistory %= M.filter (/= win)
    deleteWinObjs win
    use (pairOfGetter #focusedWin focusedWorkspace) >>= \case
      (Just fWin, Just ws) | fWin == win -> do
        use (workspaceWindows ws) >>= \case
          S.Empty -> do
            #focusedWin .= Nothing
            #workspaceFocusHistory %= M.delete ws
          h S.:<| _ -> do
            setFocusedWindowAndHistory ws h
      _ -> pure ()

dimensions :: MVar WMState -> Object RiverWindowV1 -> Int32 -> Int32 -> W ()
dimensions mvar winPtr w h = do
  modifyMVarW_ mvar $ pure . execState updateDimensions
 where
  updateDimensions =
    use (pairOfGetter #opDeltaState (#allWindows % at winPtr)) >>= \case
      (None, Just winRec) -> do
        let isFloat = view #winFloat winRec
            isFull = view #winFull winRec
        when (isFloat && not isFull) $ #allWindows % at winPtr %? #winFloatGeo %?= \r -> r{rw = w, rh = h}
      _ -> pure ()

newParent :: MVar WMState -> Object RiverWindowV1 -> Maybe (Object RiverWindowV1) -> W ()
newParent _ _ Nothing = pure ()
newParent mvar win (Just parent) = do
  modifyMVarW_ mvar $ pure . execState transform
 where
  transform = do
    #allWindows % at win %? #winParent ?= parent
    deleteWinObjs win
    use focusedWorkspace >>= \case
      Just focusedWs -> #floatingQueue % at focusedWs %?= (win :)
      Nothing -> pure ()

dimHint :: MVar WMState -> Object RiverWindowV1 -> Int32 -> Int32 -> Int32 -> Int32 -> W ()
dimHint mvar win minW minH maxW maxH = do
  modifyMVarW_ mvar $ pure . execState transform
 where
  transform = do
    #allWindows % at win %? #winDimHint .= (minW, minH, maxW, maxH)
    when (minW == maxW && minH == maxH && minW /= 0 && minH /= 0) $ do
      deleteWinObjs win
      use focusedWorkspace >>= \case
        Just focusedWs -> #floatingQueue % at focusedWs %?= (win :)
        Nothing -> pure ()

newTitle :: MVar WMState -> Object RiverWindowV1 -> Maybe T.Text -> W ()
newTitle _ _ Nothing = pure ()
newTitle mvar win (Just title) = do
  modifyMVarW_ mvar $ \state -> do
    pure $ state & #allWindows % at win %? #winTitle .~ title

newAppId :: MVar WMState -> Object RiverWindowV1 -> Maybe T.Text -> W ()
newAppId _ _ Nothing = pure ()
newAppId mvar win (Just appID) = do
  modifyMVarW_ mvar $ \state -> do
    pure $ state & #allWindows % at win %? #winAppId .~ appID

fullReq :: MVar WMState -> Object RiverWindowV1 -> Maybe (Object RiverOutputV1) -> W ()
fullReq mvar win mOutput = do
  modifyMVarW_ mvar $ pure . execState transform
 where
  transform = do
    let output = maybe nonObject id mOutput
    focusedWs <- use focusedWorkspace
    targetWs <- use (#allOutputWorkspaces % to (B.lookup output))
    let actualWs = fromMaybe 1 $ msum [targetWs, focusedWs]
    #allWindows % at win %?= (\w -> w{winFull = True, winPinned = False})
    deleteWinObjs win
    #fullscreenQueue % at actualWs %?= (win :)

exitFullReq :: MVar WMState -> Object RiverWindowV1 -> W ()
exitFullReq mvar win = do
  modifyMVarW_ mvar $ pure . execState transform
 where
  transform =
    use (pairOfGetter (#allWindows % at win) (#allWorkspacesFullscreen % to (BS.lookupA win))) >>= \case
      (Just Window{winFloat}, Just ws) -> do
        #allWindows % at win %? #winFull .= False
        #allWorkspacesFullscreen %= BS.delete win
        #manageQueue >>>= (riverWindowV1ExitFullscreen win >> riverWindowV1InformNotFullscreen win)
        if winFloat
          then #floatingQueue % at ws %?= (win :)
          else #allWorkspacesTiled %= BS.insert ws win
      _ -> pure ()

maximizeReq :: MVar WMState -> Object RiverWindowV1 -> W ()
maximizeReq mvar win = do
  modifyMVarW_ mvar $
    pure . (#manageQueue >>~ riverWindowV1InformMaximized win) . (#allWindows % at win %? #winMaximized .~ True)

unmaximizeReq :: MVar WMState -> Object RiverWindowV1 -> W ()
unmaximizeReq mvar win = do
  modifyMVarW_ mvar $
    pure . (#manageQueue >>~ riverWindowV1InformUnmaximized win) . (#allWindows % at win %? #winMaximized .~ False)
